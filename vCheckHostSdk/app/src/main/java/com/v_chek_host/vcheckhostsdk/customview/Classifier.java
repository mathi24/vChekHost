...
import com.v_chek_host.vcheckhostsdk.ClassifierQuantizedEfficientNet;
...
public abstract class Classifier {
   ...
   */
  public static Classifier create(Activity activity, Model model, Device device, int numThreads, String s)
      throws IOException {
    /*if (model == Model.QUANTIZED_MOBILENET) {
      ...
      return new ClassifierQuantizedEfficientNet(activity, device, numThreads, s);
    } else {
  ...
  private static MappedByteBuffer loadMappedFile(@NonNull Context context, @NonNull String filePath) throws IOException {
    ...
    File file = new File(Environment.getExternalStorageDirectory() + "/AIMagix/" + filePath);
  ...
  protected Classifier(Activity activity, Device device, int numThreads, String s) throws IOException {
   ...
   /* if(s.equals("old")) {
      tflite = new Interpreter(*//*tfliteModel*//*loadMappedFile(activity, "HVBlueGreen.tflite"), tfliteOptions);
      labels = FileUtil.loadLabels(activity, "labelsold.txt");
    }else {*/
      tflite = new Interpreter(/*tfliteModel*/loadMappedFile(activity, "BlueGreen.tflite"), tfliteOptions);
      labels = FileUtil.loadLabels(activity, "monroney-tflite/labels.txt");
   // }
  ...
  public List<Recognition> recognizeImage(final Bitmap bitmap, int sensorOrientation) {
    ...
    Bitmap[] rightQtrBitmap = splitBitmap(bitmap);
    Trace.beginSection("loadImage");
    long startTimeForLoadImage = SystemClock.uptimeMillis();
    inputImageBuffer = loadImage(rightQtrBitmap[1], sensorOrientation);
