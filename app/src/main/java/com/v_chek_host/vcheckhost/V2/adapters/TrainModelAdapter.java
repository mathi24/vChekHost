...
public class TrainModelAdapter extends RecyclerView.Adapter<TrainModelAdapter.MyViewHolder> {
    List<ModelData.ApiModelData> list;
    Context ctx;
    int lastitemPosition = -1;

    public TrainModelAdapter(List<ModelData.ApiModelData> list, Context ctx) {
        this.list = list;
        this.ctx = ctx;
    }
    ...
    @NonNull
    @Override
    public TrainModelAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.activity_list, parent, false);
        return new TrainModelAdapter.MyViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(@NonNull TrainModelAdapter.MyViewHolder holder, int position) {
        ModelData.ApiModelData message = list.get(position);
